    ...
    constructor( renderer ){
        ...
        function onSelectEnd( ){
            ...
            if (self.type === 'swipeY'){
                ...
                self.dispatchEvent( { type:'swipeY', direction } );
                self.type = 'unknown';
            }else if(self.type === 'swipeX'){
                const direction = (data.startPosition.x < self.controller1.position.x) ? "L2R" : "R2L";
                self.dispatchEvent( { type:'swipeX', direction } );
                self.type = 'unknown';
            }
            ...
            if (data1.startPosition !== undefined){
                if (this.multiTouch){
                    ...
                    if ( dist > 0.01 && velocity > 0.1 ){
                        ...
                        let maxX = (Math.abs(v.x) > Math.abs(v.y)) && (Math.abs(v.x) > Math.abs(v.z));
